...
function hasQueryParameter(query: OpenAICompletionsQuery | undefined, name: string): boolean {
	return query?.split("&").some(entry => entry.split("=", 1)[0] === name) ?? false;
}
...
async function createClient(
	...
	if (baseUrl?.includes(".openai.azure.com") && !hasQueryParameter(endpointQuery, "api-version")) {
		azureDefaultQuery = { "api-version": $env.AZURE_OPENAI_API_VERSION || "2024-10-21" };
	}
	const azureQueryEntries = Object.entries(azureDefaultQuery ?? {});
	const azureQuery = azureQueryEntries.length > 0 ? new URLSearchParams(azureDefaultQuery).toString() : undefined;
	...
	const wrappedFetch = Object.assign(
		async (input: string | URL | Request, init?: RequestInit): Promise<Response> => {
			const response = await baseFetch(appendQueryToRequest(input, endpointRequestQuery), init);
			if (response.ok) {
