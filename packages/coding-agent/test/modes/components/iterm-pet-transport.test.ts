...
describe("iTerm Pet transport factory", () => {
	...
	])("rejects unmanaged multiplexer contexts: %j", env => {
		const transport = createNativePetTransport({
			ui: factoryUi,
			env: { TERM_PROGRAM: "iTerm.app", TERM_PROGRAM_VERSION: "3.7", ...env },
		});
		expect(transport).toBeUndefined();
	});
	it("uses the non-destructive probe drain when the UI provides one", async () => {
		const listeners = new Set<(data: string | Uint8Array) => unknown>();
		let probeDrains = 0;
		let shutdownDrains = 0;
		const transport = createNativePetTransport({
			ui: {
				...factoryUi,
				drainPetProbeInput: async () => {
					probeDrains++;
				},
				drainInput: async () => {
					shutdownDrains++;
			...
			env: { TERM_PROGRAM: "iTerm.app", TERM_PROGRAM_VERSION: "3.7" },
		});
		expect((await transport?.probe())?.available).toBe(true);
		expect(probeDrains).toBe(1);
		expect(shutdownDrains).toBe(0);
	});
...
describe("iTerm Pet transport", () => {
	...
	it("rejects a synchronous capability reply from output.write until the written ack, then accepts post-ack input", async () => {
		const clock = new Clock();
		const input = new Input();
		const write = Promise.withResolvers<{ status: "written" }>();
		const output = {
			write: async () => {
				input.send(ack);
		...
		const resolveWrite = write.resolve;
	...
	it("fails when lifecycle or client validity changes during a cursor query", async () => {
		for (const change of ["lifecycle", "client"] as const) {
			const cursorQuery = Promise.withResolvers<{ status: number; stdout: string }>();
			const release = cursorQuery.resolve;
			...
			const x = await managedReady(
				async argv => {
					calls.push([...argv]);
					if (argv[0] === "show-options" && argv[1] === "-q") return { status: 0, stdout: "" };
					if (argv[0] === "display-message") return cursorQuery.promise;
					return { status: 0, stdout: "on" };
				},
	...
	it("restores managed pane options when a later client query fails", async () => {
		let listClientsAvailable = true;
		const calls: string[][] = [];
		const tmux: PetTmuxRunner = async argv => {
			calls.push([...argv]);
			if (argv[0] === "list-clients")
				return listClientsAvailable ? { status: 0, stdout: "client-1\t/dev/ttys010\n" } : { status: 1, stdout: "" };
			if (argv[0] === "display-message") return { status: 0, stdout: "%1\n" };
		...
		listClientsAvailable = false;
		expect((await x.transport.inspectManagedTopology()).reason).toBe("topology-ineligible");
	...
	it("serializes revoke restore behind deferred managed preparation", async () => {
		...
		const tmux: PetTmuxRunner = async (argv: readonly string[]) => {
			...
			if (argv[0] === "set-option" && argv[1] === "-p") {
				const deferred = Promise.withResolvers<PetTmuxResult>();
				if (argv.at(-1) === "on") releasePrepare = deferred.resolve;
				else releaseRestore = deferred.resolve;
	...
	it("restores before retrying after managed preparation rejection", async () => {
		...
		const tmux: PetTmuxRunner = async (argv: readonly string[]) => {
			...
			if (argv[0] === "set-option" && argv[1] === "-p" && argv[4] === "allow-passthrough" && argv[5] === "off") {
				const deferred = Promise.withResolvers<PetTmuxResult>();
				releaseRestore = deferred.resolve;
				return deferred.promise;
			}
			return { status: 0, stdout: "on" };
		};
	...
	it("ignores stale topology completion without probing or mutating identity", async () => {
		const topology = Promise.withResolvers<{
			clients: number;
			paneId: string;
			ownedPaneId: string;
			clientId: string;
		}>();
		const release = topology.resolve;
		const x = make({
			mode: "managed",
			paneId: "%1",
			sessionTarget: "s",
...
it("coalesces concurrent restores while preserving the newer topology reason", async () => {
	const calls: string[][] = [];
	const restore = Promise.withResolvers<PetTmuxResult>();
	const release = restore.resolve;
	const x = make({
		mode: "managed",
		paneId: "%1",
		sessionTarget: "s",
