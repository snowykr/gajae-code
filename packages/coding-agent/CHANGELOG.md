...
- Added an Extragoal local skill template (`docs/extragoal-skill-template.md`) documenting an external final review gate on top of `ultragoal` — a fresh-context, cross-family, tool-restricted read-only reviewer with a machine-parsable verdict contract, mandatory bundle secret scan, prompt-injection stance, explicit findings triage, and a bounded re-sign loop — the default reviewer lane now authors verdicts with `openai-codex/gpt-5.5:xhigh`, and an opt-in, operator-local maximalist N-of-N reviewer recipe runs several independent reviewers on the same immutable bundle under a mechanical AND-gate (native `gpt-5.5:xhigh`/`anthropic/claude-fable-5:xhigh` lanes plus external reference adapters such as GPT-5.5 Pro via `insane-review`) with no new GJC core dependencies. Ships with a `reviewer` stance profile and cross-session review-gate recipe in `docs/multi-vendor-profiles.md`, pinned by `test/extragoal-template.test.ts`.
...
- Telegram mirror messages now render as native Bot API 10.1 rich messages (tables, headers, code blocks, checklists, quotes, math) instead of the HTML approximation, on by default with a global `/rich on|off` toggle. The public setting is `notifications.telegram.rich.enabled` (default `true`); the OFF state emits zero rich Bot API calls and byte-identical HTML/media request bodies, so it is a no-op regression-wise. Rich promotion is fail-closed: only a finalized, non-editable `sendMessage` carrying non-empty final-answer markdown is promoted, and any Bot API miss (`ok:false`/throw) falls back to the unchanged chunked HTML path. Action prompts (`ask`/`idle`) render rich with an inline keyboard for asks (reply routing preserved on both rich-success and HTML fallback); `idle` is notify-only. Streamed (editable) finals stay on `editMessageText` and never rich-promote or duplicate — split live previews never fan out, split continuations strip rich markers, a late `message_update` after `turn_end` is dropped, and pool flushes are serialized. `/rich on|off` is handled daemon-local at paired-chat pre-routing (works with no session and is never consumed as an ask answer), persists atomically, and leaves runtime state unchanged on a persistence failure. Opt-in `sendRichMessageDraft` live-preview streaming is gated behind `rich.enabled` with daemon-global draft ids, and replying to a rich message restores its source markdown as context (Telegram does not echo rich bodies back).
