...
const inputNameValue = inputName ?? path.join(output, "manifest.json");
...
const canonicalPath = async (candidate: string): Promise<string> => {
	...
	while (!(await pathExists(existing))) {
		...
		if (parent === existing) die(`path has no existing ancestor: ${candidate}`);
		suffix.unshift(existing.slice(parent.length + 1));
		existing = parent;
	}
	return suffix.reduce((current, part) => path.join(current, part), await fs.realpath(existing));
};
const safePath = async (root: string, declared: string): Promise<string> => {
	if (!declared || path.isAbsolute(declared)) die(`member path is unsafe: ${declared}`);
	const candidate = path.resolve(root, declared);
	const rel = path.relative(root, candidate);
	if (!rel || rel.startsWith("..") || path.isAbsolute(rel)) die(`member path escapes root: ${declared}`);
	if (!(await pathExists(candidate))) die(`member unavailable: ${declared}`);
	const stat = await fs.lstat(candidate);
	if (stat.isSymbolicLink() || !stat.isFile()) die(`member is not a regular file: ${declared}`);
	const canonical = await fs.realpath(candidate);
	const canonicalRel = path.relative(await fs.realpath(root), canonical);
	if (!canonicalRel || canonicalRel.startsWith("..") || path.isAbsolute(canonicalRel))
		die(`member path escapes root: ${declared}`);
	return canonical;
};
const parseJsonFile = async (path: string, message: string): Promise<Json> => {
	try {
...
const digestMember = async (root: string, value: unknown, label: string, caseId?: string): Promise<Member> => {
	...
	const path = await safePath(root, object.path);
...
const validateTerminalContent = async (
	rootDir: string,
	textPath: string,
	ansiPath: string,
	bundle: Bundle,
	...
	const text = (await Bun.file(await safePath(rootDir, textPath)).text()).normalize("NFC");
	const ansi = await Bun.file(await safePath(rootDir, ansiPath)).text();
...
const normalizeBundle = async (
	...
	for (let index = 0; index < membersRaw.length; index++)
		members.push(await digestMember(rootDir, membersRaw[index], `${version}/${mode}/${caseId}/${index}`, caseId));
	const names = new Set(members.map(member => path.basename(member.path)));
	if (names.size !== 4 || memberNames.some(name => !names.has(name)))
		die(`${version}/${mode}/${item.caseId}: required evidence members are missing`);
	const metadataMember = members.find(member => path.basename(member.path) === "metadata.json");
	if (!metadataMember) die(`${version}/${mode}/${caseId}: required metadata member is missing`);
	const metadata = await parseJsonFile(
		await safePath(rootDir, metadataMember.path),
	...
	const textMember = members.find(member => path.basename(member.path) === "terminal.txt");
	const ansiMember = members.find(member => path.basename(member.path) === "terminal-ansi.txt");
	if (!textMember || !ansiMember) die(`${version}/${mode}/${caseId}: required terminal members are missing`);
	await validateTerminalContent(
		rootDir,
		textMember.path,
		ansiMember.path,
		bundle,
...
const writeJournal = async (path: string, value: Json): Promise<void> => {
...
const parseRoot = async (path: string): Promise<Json> => parseJsonFile(path, "capture input is not valid JSON");
...
const validatePublished = async (outputPath: string): Promise<void> => {
	if (!(await pathExists(outputPath)) || (await fs.readdir(outputPath)).sort().join() !== requestedVersions.join())
		die("published output hierarchy is invalid");
	for (const version of requestedVersions)
		for (const mode of requestedModes) {
			const dir = path.join(outputPath, version, mode);
			if (
				!(await pathExists(dir)) ||
				(await fs.readdir(dir)).sort().join() !== "captures,evidence.json,manifest.json,rasters"
			)
				die(`published hierarchy is invalid: ${version}/${mode}`);
			const manifest = await parseRoot(path.join(dir, "manifest.json"));
			const evidence = await parseRoot(path.join(dir, "evidence.json"));
			if (
			...
			for (const entry of files) {
				...
				const path = await safePath(dir, member.path);
				const bytes = await Bun.file(path).bytes();
			...
			const walk = async (base: string, prefix: string): Promise<void> => {
				for (const name of await fs.readdir(base)) {
					...
					if ((await fs.lstat(entryPath)).isDirectory()) await walk(entryPath, rel);
					else actual.push(rel);
				}
			};
			await walk(path.join(dir, "captures"), "captures");
			await walk(path.join(dir, "rasters"), "rasters");
				...
				for (const name of memberNames) {
					const memberPath =
						typeof b[`${name.replace(".", "_")}Path`] === "string"
							...
							: `${path.dirname(metadataPath)}/${name}`;
					if (memberPath !== `${path.dirname(metadataPath)}/${name}` || !listed.has(memberPath))
						die(`published bundle member is unbound: ${version}/${mode}`);
					if (name === "terminal.txt") textPath = memberPath;
					if (name === "terminal-ansi.txt") ansiPath = memberPath;
				}
				await validateTerminalContent(dir, textPath, ansiPath, bundle, metadata, `${version}/${mode}/${key}`);
			}
...
const recoverPublication = async (outputPath: string): Promise<void> => {
	const journal = journalPathFor(outputPath);
	if (!(await pathExists(journal))) return;
	...
	const declaration = await parseRoot(journal);
	const backup = typeof declaration.backup === "string" ? declaration.backup : "";
	const validSibling = async (value: string): Promise<boolean> => {
		if (!path.isAbsolute(value) || path.resolve(value) !== value || path.dirname(value) !== path.dirname(outputPath))
			return false;
		if (value === outputPath || value === journal) return false;
		const name = path.basename(value);
		if (!name.startsWith(`${path.basename(outputPath)}.backup-`) || !(await pathExists(value))) return false;
		const stat = await fs.lstat(value);
		return (
			stat.isDirectory() &&
			!stat.isSymbolicLink() &&
			(await canonicalPath(path.dirname(value))) === (await canonicalPath(path.dirname(outputPath)))
		);
	};
	if (declaration.output !== outputPath || (backup && !(await validSibling(backup))))
		die("publication journal is invalid");
	if (!(await pathExists(outputPath)) && backup) {
		await validatePublished(backup);
		await fs.rename(backup, outputPath);
	}
	if (await pathExists(outputPath)) {
		await validatePublished(outputPath);
		await fs.rm(journal, { force: true });
	}
};
const compareRaster = async (scenario: StateScenario, artifacts: Artifact[], rootDir: string): Promise<void> => {
	const refs = new Map(artifacts.map(ref => [ref.sha256, ref]));
	const raster = async (value: RasterEvidence): Promise<{ width: number; height: number; bytes: Buffer }> => {
		const ref = refs.get(value.artifactSha256);
		if (ref?.format !== "rgba8") throw Error("raster must reference rgba8 sidecar");
		const bytes = Buffer.from(await Bun.file(await safePath(rootDir, ref.path)).bytes());
	...
	const ex = await raster(scenario.expected);
	const ac = await raster(scenario.actual);
	const before = await raster(scenario.erase.before);
	const after = await raster(scenario.erase.after);
...
const validateArtifacts = async (capture: Json, rootDir: string): Promise<Artifact[]> => {
	...
	for (const value of raw) {
		...
		seen.add(item.path);
		const path = await safePath(rootDir, item.path);
		const bytes = Buffer.from(await Bun.file(path).bytes());
...
async function main(): Promise<void> {
	const rootInput = path.resolve(inputNameValue);
	const inputFile =
		(await pathExists(rootInput)) && !rootInput.endsWith(".json") ? path.join(rootInput, "manifest.json") : rootInput;
	if (!inputName && !(await pathExists(inputFile))) {
		await recoverPublication(path.resolve(output));
		await validatePublished(path.resolve(output));
		process.exit(0);
	}
	if (!(await pathExists(inputFile))) die(`required capture input unavailable: ${inputFile}`);
	const captureRoot = inputFile.endsWith(".json") ? path.dirname(inputFile) : inputFile;
	const canonicalCaptureRoot = await canonicalPath(captureRoot);
	const canonicalOutput = await canonicalPath(output);
	const relativeOutput = path.relative(canonicalCaptureRoot, canonicalOutput);
	const relativeCapture = path.relative(canonicalOutput, canonicalCaptureRoot);
	if (
		inputName &&
		(!relativeOutput ||
	...
	const root = await parseRoot(inputFile);
	if (
	...
	for (const version of requestedVersions)
		for (const mode of requestedModes) {
			...
			for (const value of rawBundles)
				bundles.push(await normalizeBundle(captureRoot, value, version, mode, classification));
			...
			for (const bundle of bundles) {
				...
				if (
					...
					(bundle.caseId === "cjk-mixed-preview-scroll" &&
						(bundle.scroll !== "top"
							? bundle.viewport !== "80x24"
							: !["80x24", "40x12"].includes(bundle.viewport)))
				)
			...
			const artifacts = await validateArtifacts(capture, captureRoot);
			...
			for (const skin of ["red", "blue"])
				for (const state of ["idle", "working", "burst", "preview"]) {
					...
					try {
						await compareRaster(scenario, artifacts, captureRoot);
					} catch (error) {
	...
	await recoverPublication(outputPath);
	const stage = await fs.mkdtemp(`${outputPath}.staging-`);
	try {
		for (const item of evidence) {
			const out = path.join(stage, item.version, item.mode);
			await fs.mkdir(path.join(out, "captures"), { recursive: true });
			await fs.mkdir(path.join(out, "rasters"), { recursive: true });
			const files: Json[] = [],
				manifestBundles: Json[] = [];
			const addFile = async (source: string, target: string, kind: string, caseId?: string): Promise<void> => {
				const bytes = await Bun.file(source).bytes();
				await Bun.write(path.join(out, target), bytes);
				files.push({
					path: target,
					sha256: sha256(bytes),
					size: bytes.length,
			...
			for (const ref of item.artifacts) {
				...
				if (!copied.has(target)) {
					const bytes = await Bun.file(await safePath(captureRoot, ref.path)).bytes();
					await Bun.write(path.join(out, target), bytes);
					files.push({
						path: target,
						sha256: ref.sha256,
			...
			for (const bundle of item.bundles) {
				...
				await fs.mkdir(path.join(out, base), { recursive: true });
				const paths: Record<string, string> = {};
				for (const member of bundle.members) {
					const name = path.basename(member.path);
					const target = `${base}/${name}`;
					await addFile(
						await safePath(captureRoot, member.path),
						target,
						name === "metadata.json"
							? "metadata"
			...
			await Bun.write(path.join(out, "evidence.json"), `${JSON.stringify(evidenceJson, null, 2)}\n`);
			const manifest = {
				schemaVersion: 2,
				expectedSha,
				gitRevision: expectedSha,
	...
	await writeJournal(journalPath, { output: outputPath, backup });
	let moved = false;
	try {
		if (await pathExists(outputPath)) {
		...
		await fs.rename(stage, outputPath);
		if (moved) await fs.rm(backup, { recursive: true, force: true });
		await fs.rm(journalPath, { force: true });
	} catch (error) {
		if (await pathExists(outputPath)) await fs.rm(outputPath, { recursive: true, force: true });
		if (moved && (await pathExists(backup))) await fs.rename(backup, outputPath);
